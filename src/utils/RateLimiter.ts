...
export class RateLimiter {
  ...
  private constructor(provider: RPCProvider, config?: Partial<RateLimitConfig>) {
    ...
    this.limiter = new Bottleneck({
      maxConcurrent: this.config.maxConcurrent ?? null,
      minTime: this.config.minTime ?? 0,
      reservoir: this.config.reservoir ?? null,
      reservoirRefreshInterval: this.config.reservoirRefreshInterval ?? null,
      reservoirRefreshAmount: this.config.reservoirRefreshAmount ?? null,
